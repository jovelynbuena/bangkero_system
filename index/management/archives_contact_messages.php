...
if (isset($_GET['delete_perm'])) {
    ...
    if ($aid > 0) {
        ...
        if ($stmt_delete->execute()) {
            header('Location: archives_contact_messages.php?deleted=1');
        } else {
            header('Location: archives_contact_messages.php?error=delete_failed');
        }
        $stmt_delete->close();
        exit();
    }
...
$where_clause = "";
if ($search !== '') {
    $where_clause = "WHERE name LIKE '%$search_safe%' OR email LIKE '%$search_safe%' OR message LIKE '%$search_safe%'";
}

$sql = "SELECT * FROM contact_messages_archive $where_clause ORDER BY archived_at DESC";
...
<head>
<meta charset="UTF-8">
<title>Archived Contact Messages | Admin Panel</title>
...
<div class="main-content">
    <div class="page-header">
        <h2><i class="bi bi-archive-fill me-2"></i>Archived Messages</h2>
        <p>View, restore or permanently delete archived contact messages</p>
    </div>
                    ...
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><small><?= nl2br(htmlspecialchars(substr($row['message'], 0, 100))) ?>...</small></td>
                                <td><?= date('M d, Y g:i A', strtotime($row['archived_at'])) ?></td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        <button onclick="confirmRestore(<?= $row['archive_id'] ?>, '<?= addslashes($row['name']) ?>')" class="btn-restore" title="Restore"><i class="bi bi-arrow-counterclockwise"></i></button>
                                        <button onclick="confirmDelete(<?= $row['archive_id'] ?>, '<?= addslashes($row['name']) ?>')" class="btn-delete" title="Delete Permanently"><i class="bi bi-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="5" class="text-center py-5 text-muted">No archived messages found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
function confirmRestore(id, name) {
    Swal.fire({
        ...
        text: `Move the message from "${name}" back to active list?`,
        icon: 'question',
...
function confirmDelete(id, name) {
    Swal.fire({
        ...
        text: `This will permanently delete the message from "${name}". This cannot be undone!`,
        icon: 'warning',
        showCancelButton: true,
...
<?php if (isset($_GET['restored'])): ?>
    Swal.fire({ icon: 'success', title: 'Restored!', text: 'Message moved to active list.', timer: 2000, showConfirmButton: false });
<?php endif; ?>
